  ...
  message.channel.send("ESPERA...").then(m => {
    ...
    m.awaitReactions(filtro, { max: 1, time: 1000, errors: ["time"] }).catch(
      () => {
        ...
        m.edit(
          "Verificate! Si No Lo Haces en 10 segundos se cansela y no puedes tener coronavirus"
        );
        ...
        m.awaitReactions(filtro, { max: 1, time: 10000, errors: ["time"] })
          ...
          .then(coleccionado => {
            ...
            if (reaccion.emoji.name === "✅") {
              message.member.roles.add("742159635078709381");
              m.edit("Tienes coronavirus ! Contagia gente");

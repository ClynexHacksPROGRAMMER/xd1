...
module.exports = async (client, channel) => {
  ...
  if (!opciones.has(`${channel.guild.id}.logs`)) return; //si en la db opciones no hay un canal logs en el clave-split .logs
  let logchannel = await opciones.get(`${channel.guild.id}.logs`); // obtiene la id del canal logs
